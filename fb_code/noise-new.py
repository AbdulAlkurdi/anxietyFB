...
if __name__ == '__main__':
    ...
    now = datetime.today().strftime('%Y-%m-%d--%H-%M-%p')
    GLOBAL_LOGGER.info(f'{now}. snrs: {snrs}.')
    
    for snr in snrs:
        now = datetime.today().strftime('%Y-%m-%d--%H-%M-%p')
        GLOBAL_LOGGER.info(f'{now}. Processing data for snr {snr}...')
        for subject in subject_ids:
            now = datetime.today().strftime('%Y-%m-%d--%H-%M-%p')
            GLOBAL_LOGGER.info(str(now)+ '. Processing data for S'+str(subject)+'...snr '+str(snr)+'...')
            make_patient_data_wnoise(subject, snr, n_samples)
